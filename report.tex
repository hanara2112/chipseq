...
\begin{center}\small
\textbf{Code:}~\href{\repourl}{\texttt{github.com/hanara2112/chipseq}}\quad\textbar\quad
\textbf{Bonus notebook (live):}~\href{\notebookurl}{\texttt{hanara2112.github.io/chipseq/.../extra\_enhancer\_analysis.html}}
\end{center}
\vspace{-0.4em}
...
The data set assigned for this report is an H3K27ac ChIP-seq library (sample S295) and a matched IgG isotype-control library (S286) from IL-7-stimulated mouse T-cells, each with $\approx\!10^{6}$ read pairs against the mm10 reference genome. The aim is to identify regions of significant H3K27ac enrichment, demonstrate that they exceed the non-specific background measured by IgG, characterise their distribution across the genome, and interpret these results in terms of promoter and enhancer biology.
...
\caption{Representative IGV view at \textbf{<INSERT GENE / chr:start--end FROM THE SCREENSHOT RULER>}. Top: H3K27ac (S295) RPKM coverage showing a broad pile-up over the called peak. Bottom: IgG (S286) RPKM coverage on the same locus, flat. The peak interval shown is one of the strongest H3K27ac peaks (fold-enrichment $>$13$\times$ over IgG).}
\label{fig:igv}
\end{figure}
